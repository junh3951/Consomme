...
def generate_trending_titles(titles, chosen_keyword, search_keyword):
    messages = [
        {"role": "system", "content": 
        "###임무### 너는 다양한 콘텐츠 소재를 상황과 트렌드에 기반하여 생성해주는 것이 너의 역할이야. 반드시 출력규칙을 준수해. \
            ...
            ###출력규칙### **소재:** 와 **소재 추천 이유:** 가 반드시 문자 그대로 포함되어야함. 번호는 붙이지않아야함. \
...
def search_videos():
    ...
    vectorizer = TfidfVectorizer(
        preprocessor=clean_word,
        tokenizer=lambda x: x.split(),
        token_pattern=None,
        stop_words=[search_keyword, category]
    )
    tfidf_matrix = vectorizer.fit_transform(titles_category)
    ...
    top_keywords = [word for word, score in tfidf_scores_sorted[:15]]
